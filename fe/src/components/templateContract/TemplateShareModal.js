...
function TemplateShareModal({ show, onClose, templateId, templateName }) {
    ...
    const handleShare = async () => {
        ...
        if (!templateId) {
            setError("Không tìm thấy ID mẫu tài liệu để chia sẻ");
            return;
        }

        setLoading(true);
        try {
            await contractService.shareTemplateContract(templateId, validEmails);
            ...
            setError("Không thể chia sẻ mẫu tài liệu. Vui lòng thử lại.");
            console.error(err);
        } finally {
            setLoading(false);
        }
        ...
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                <div className="modal-header">
                    <h3>Chia sẻ mẫu tài liệu</h3>
                    <button className="close-btn" onClick={onClose}>×</button>
                </div>
                <div className="modal-body">
                    <p className="share-info">
                        <strong>Mẫu tài liệu:</strong> {templateName}
                    </p>
                    <p className="share-note">
                        <em>Lưu ý: Thường chỉ chia sẻ các mẫu đã sẵn sàng sử dụng.</em>
                    </p>
